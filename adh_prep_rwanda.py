# -*- coding: utf-8 -*-
"""
Created on Wed Sep 21 08:30:58 2022
...
def execute(data_path, country):
    ...
    '''
    tables = tabula.read_pdf("{}.pdf".format(data_path), pages=(9), stream=True)
    ...
    df = tables[0]
    df = df.drop([0,1,4,5,6,7,8,22,23])
    #keeptemp  =  df['Annex 3: Consumer Price Index, All Rwanda'].str.split('\s{2,}', expand = True)
    df[['code','divisions']]  =  df['Annex 3: Consumer Price Index, All Rwanda'].str.split(' ',1, expand = True)
    df = df.loc[:,['divisions','Unnamed: 0']]

    # labels
    df_labels = df.iloc[:,[0]]
    template = [0,1,2,3,4,5,5,5,6,7,8,9,10,11,12]
    ...
    df[['col1','col2','col3','col4',last]] = df['Unnamed: 0'].str.split(' ',5, expand = True)
    df = df.loc[:,['divisions',last]]
